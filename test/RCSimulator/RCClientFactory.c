...
void RCClientFactoryFromCfgFile(const char *pConfigFileName, ClientListObject_t *pClientList, ProcedureListObject_t *pProcedureList, ServoControllerObject_t *pServoControllerData)
{
	...
	if(pBuffer)
	{
		  ...
		  if (jobj)
		  {
			  ...
			  *pServoControllerData = NewServoControllerDataJson(jobj);
...
static BOOL ServoController_MonCmd(ClientListObject_t clientlist, char * cmdLine)
{
	...
	if ((argc == 2) && (0 == strcmp(argv[1], pCmdServoControllerSetIp)))
	{
	}
	else
	{
		MON_WriteInfof("Usage:");
		MON_WriteInfof("\n%s\t%s <servocontroller-ip>", pCmdServoController, pCmdServoControllerSetIp);
	}
	return TRUE;
}


void RCClientFactoryInteractive(ClientListObject_t *pClientList, ProcedureListObject_t *pProcedureList, ServoControllerObject_t *pServoControllerData)
{
	*pClientList = NewClientList(NULL);
	*pServoControllerData = NewServoControllerData(NULL);
	*pProcedureList = NULL;

	MON_AddMonCmd(pCmdClient, Client_MonCmd, *pClientList );
	MON_AddMonCmd(pCmdServoController, ServoController_MonCmd, *pServoControllerData);
}
